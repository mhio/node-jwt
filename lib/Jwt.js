...
class Jwt {
  ...
  async rotateRsaKeyPair() {
    ...
    this.jwt_public_key = public_key
  ...
  async koaVerifyJwtCookieMiddleware(ctx, next) {
    ...
    const cookeie_token_data = await this.verify(cookie_token_string)
    ctx.state.cookie_token = cookeie_token_data // eslint-disable-line require-atomic-updates
   ...
   * @returns {string} - Parsed token
   */
  static getJwtTokenFromKoaContextCookie(context) {
    if (!context) {
      throw new WebException('No context', { status: 400 })
    }
    if (!context.cookies) {
    ...
    const jwt_cookie = context.cookies.get(this.options.koa_cookie)
    if (!jwt_cookie) {
      throw new WebException('Authentication Error', { status: 403, code: 'authorization_cookie_missing' })
    }
    debug('context.cookies "%s"', this.options.koa_cookie, jwt_cookie)
    // return just the matched token
    return jwt_cookie
  }
