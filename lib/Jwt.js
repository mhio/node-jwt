...
const fs = require('fs')
const debug = (require('debug')('mh:jwt:Jwt').enabled)
  ? require('debug')('mh:jwt:Jwt')
  : function noop() { }

const Promise = require('bluebird')
const jwt = require('jsonwebtoken')

const { Exception, WebException }  = require('@mhio/exception')

class Jwt {

  static initialiseClass() {
    this.environment_var_name = 'JWT_SECRET'
  }
  ...
  /**
   * Helper if you just want to `app.use(Jwt.koaMiddleware(options))`
   * See .constructor() options
   * @return {function} Koa middleware function to verify JWT header
   */
  static koaMiddleware(options) {
    const jwt_instance = new this(options)
    return jwt_instance.koaMiddleware()
  }
  ...
  constructor(options) {
    ...
    if (options) {
      ...
      if (options.jwt_public_key) this.jwt_verify_secret = options.jwt_public_key
      if (options.jwt_private_key_path) this.jwt_private_key_path = options.jwt_private_key_path
      if (options.jwt_public_key_path) this.jwt_public_key_path = options.jwt_public_key_path
    }
  ...
  async init() {
    ...
    if (['HS256', 'HS384', 'HS512'].includes(this.jwt_algorithm)) {
      ...
      if (!this.jwt_sign_secret) {
        this.jwt_sign_secret = process.env[this.constructor.environment_var_name] // JWT_SECRET
      }
      ...
      if (this.jwt_private_key_path || this.jwt_public_key_path) {
        this.jwt_sign_secret = fs.readFileSync(this.jwt_private_key_path)
        this.jwt_verify_secret = fs.readFileSync(this.jwt_public_key_path)
      }
      if (!this.jwt_sign_secret && !this.jwt_verify_secret ) {
        // last thing is to generate an ephemeral keypair
        await this.rotateRsaKeyPair()
      }
      if (!this.jwt_sign_secret) {
        throw new Error('Supply a private key via `jwt_private_key`')
      }
      if (!this.jwt_verify_secret) {
  ...
  /**
   * Set a defailt options for the nodewebtoken `.verify` call
  ...
  /**
   * Set a defailt options for the nodewebtoken `.sign` call
   * https://github.com/auth0/node-jsonwebtoken
   * @param {string} name     - Option key name
   * @param {*} value         - Option value
   */
  ...
  async koaVerifyJwtMiddleware(ctx, next) {
    const token_string = this.constructor.getJwtTokenFromKoaContext(ctx)
  ...
  /**
   * Creates a new ephemeral JWT key pair.
   * Maybe store the old one's for a bit so people don't get booted.
   * @return {array.<{private_key: String, public_key: String}>} Array of private and public RSA key
   */
  async rotateRsaKeyPair() {
    //this.last_jwt_sign_secret = this.jwt_sign_secret
    //this.last_jwt_verify_secret = this.jwt_verify_secret
    const [private_key, public_key] = await this.constructor.generateRsaKeyPair()
    debug('generated new RSA public key and private key')
...
module.exports = { Jwt, WebException, default: Jwt }
